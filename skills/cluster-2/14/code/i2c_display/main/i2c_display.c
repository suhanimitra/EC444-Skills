/*
  Adapted I2C example code to work with the Adafruit 14-segment Alphanumeric Display. Key notes: MSB!!
    ...
    Some code is also from Blink Example and hardware interrupts.
*/
...
static void IRAM_ATTR gpio_isr_handler(void* arg) {
    ...
    counting = true; // Start counting when button is pressed
}
...
static void test_alpha_display() {
    ...
    while (1) {
        ...
        //Used ChatGPT for aid with this section of the code
        // If counting is active, display counter (mod 16)
        if (counting) {
            int modCounter = counter % 16;
            
            // Separate modCounter into tens and ones place
            uint8_t tens = modCounter / 10;  // Get tens place digit
            uint8_t ones = modCounter % 10;  // Get ones place digit

            // Use numtable to convert digits to displayable format
            displaybuffer[2] = numtable[tens];  // Display tens place
            displaybuffer[3] = numtable[ones];  // Display ones place
            ...
            // Increment counter and wrap around modulo 16
            counter++;
        }

        vTaskDelay(1000 / portTICK_PERIOD_MS); // 1 second delay for each counter increment
    }
}
