...
static const adc_atten_t atten = ADC_ATTEN_DB_6;        //0dB attenuation fix for ADC scale
...
void app_main(void)
{
    ...
    while (1) {
        ...
        float thermistor_r = Ro * (3300.0 / voltage - 1.0);
        ...
        printf("Raw: %ld\tVoltage: %ldmV\t     Temperature: %fC\n", adc_reading, voltage, celcius);
